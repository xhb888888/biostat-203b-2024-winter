...
d_items <- read_csv("~/mimic/icu/d_items.csv.gz") |>
  filter(str_detect(category, "Vital Signs"))
...
d_items <- d_items |> 
  filter(str_detect(label, 
                    "Heart Rate|respiratory|Non Invasive Blood Pressure systolic|Non Invasive Blood Pressure diastolic|Temperature Fahrenheit")) |>
  select(itemid, label)
```
...
ggplot(chartevents, aes(x = charttime, y = value, color = label)) +
  geom_line() +
  facet_grid(label ~stay_id, scales = "free") +
...
```{r}
# I want to calculate the differenece of minutes between storetime in 
# labevent_tble and intime in icustays_tble based on the same subject_id.
labevents_tble <- arrow::open_dataset("./labevents.parquet") |>
  ...
  select(subject_id, itemid, value, valuenum, storetime)
