...
**Answer:** The runtime is faster. It takes 0.903 second compared to 1.016 seconds which is calling without specifying column data types. The resultant tibble uses 79.46 MB memory.
...
**Answer:** It takes 0.019 seconds in ingest+select+filter process. There are 24855909 rows. The first 10 rows matches with those in Q2.3.
...
**Answer:** The Parquet file is about 2.07 GB. It takes 0.164 seconds to ingest+select+filter the Parquet file. The number of rows is 24855909. The first 10 rows of the result tibble match those in Q2.3. Here are codes to generate these results.
...
**Answer:** It takes 0.202 seconds to ingest+convert+select+filter the Parquet file. The number of rows is 24855909. The first 10 rows after sorting the result tibble will match those in Q2.3. However, it might not match exactly if I do not sort here. Parallel processing causes uncertainty in output which sometimes change the order of rows. Here are codes to generate these results.
