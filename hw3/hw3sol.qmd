...
patients %>% 
  ...
  left_join(admission, by = "subject_id") %>%
  left_join(transfers, by = "subject_id") %>%
  left_join(procedures_icd, by = "subject_id") %>% 
  left_join(diagnoses_icd, by = "subject_id") %>% 
  head()
```

```{r}
d_icd_diagnoses |> 
  filter(icd_code == 1217)
```
