...
**Answer:** It takes 0.129 seconds to ingest+convert+select+filter the Parquet file. The number of rows is 24855909. The first 10 rows of the result tibble match those in Q2.3. Here are codes to generate these results.

```{r}
#| eval: false

arrow::open_dataset('~/labevents.parquet') |> 
  arrow::to_duckdb('~/labevents.db')
```
