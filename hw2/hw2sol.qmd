...
'fread' is the fastest. The default parsed data types are same. All three functions expect to be passed in 'csv' file, which will make them most efficient. However, they can also read in other file format or compressed file with the cost of efficiency. The resultant dataframe by 'read.csv' uses 158.71 MB memory. The resultant tipple by 'read_csv' uses 55.31 MB memory. uses 1.2GB memory. The resultant dataframe by 'fread' uses 50.13 MB memory.
...
**Answer:** The runtime changes a little bit. It takes 1.62 second compared to 1.58 seconds with calling without specifying column data types. The resultant tibble uses 79.46 MB memory.
...
**Answer:** It takes 0.017 seconds in ingest+select+filter process. There are 24855909 rows. The first 10 rows matches with those in Q2.3. 
...
**Answer:** The Parquet file is 96 Bytes. It takes 0.146 seconds to ingest+select+filter the Parquet file. The number of rows is 24855909. The first 10 rows of the result tibble match those in Q2.3. Here are codes to generate these results.
...
Here is the size of the file (Unit: Bytes):
...
**Answer:** It takes 0.031 seconds to ingest+convert+select+filter the Parquet file. The number of rows is 24855909. The first 10 rows after sorting the result tibble will match those in Q2.3. However, it might not match exactly if I do not sort here. Parallel processing causes uncertainty in output which sometimes change the order of rows. Here are codes to generate these results.
...
**Answer:** DuckDB is a database management system that is designed to be fast and efficient. It's like a super organized library for data, where you can quickly find and retrieve the information you need.
