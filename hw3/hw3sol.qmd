...
labevents <- arrow::open_dataset("./labevents.parquet")
...
ggplot(chartevents, aes(x = charttime, y = value, color = abbreviation)) +
  geom_point() +
...
```{r}
# The store time is based on the intime of the icustays_tble. For each icu stay i want the record of the last store time before the intime of each icu stay.
labevents_tble <- arrow::open_dataset("./labevents.parquet") |>
  ...
  dplyr::mutate(itemid = dplyr::case_when(
  ...
  dplyr::select(subject_id, itemid, valuenum, charttime) |>
  collect()
```

```{r}
# check for each chartime if there is same itemid
arrow::open_dataset("./labevents.parquet") |>
  dplyr::filter(itemid %in% c(50912, 50971, 50983, 50902, 50882, 
                       51221, 51301, 50931)) |>
  dplyr::filter(subject_id == 10171405) |>
  arrange(charttime, itemid) |>
  collect()

  #count appearance if there is same itemid appear twice
  
```


```{r}
labevents_tble_temp <- labevents_tble |> 
  dplyr::group_by(subject_id, charttime, itemid) |>
  pivot_wider(names_from = itemid, 
              values_fn = list(valuenum = mean), 
              values_from = valuenum, id_cols = c(subject_id, charttime))
```


```{r}
labevents_tble_temp <- labevents_tble_temp |>
  arrange(subject_id, charttime)
```


```{r}
icustays_tble_temp <- icustays_tble |>
  select(subject_id, stay_id, intime, outtime)
```

```{r}
combined_data <- icustays_tble_temp |>
  left_join(labevents_tble_temp, by = "subject_id")
```

```{r}
#remove intime outtime charttime
labevents_tble <- combined_data |>
  arrange(subject_id, intime, charttime) |>
